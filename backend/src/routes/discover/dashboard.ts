...
router.get('/', asyncHandler(async (req: AppRequest, res: Response) => {
  ...
  // Check if request is from a custom domain (use tenant context, not header)
  const fromDomain = tenant?.fromDomain === true && tenant?.resolvedFrom === 'domain';

  let dashboard;
  if (fromDomain && tenant?.appId && tenant?.tenantId && tenant?.tenantId !== '00000000-0000-0000-0000-000000000000') {
...
router.get('/live', asyncHandler(async (req: AppRequest, res: Response) => {
  ...
  const result = await dashboardService.getLiveTickets(
    pagination,
    fromDomain && tenant?.tenantId !== '00000000-0000-0000-0000-000000000000' ? tenant?.appId : undefined,
    fromDomain && tenant?.tenantId !== '00000000-0000-0000-0000-000000000000' ? tenant?.tenantId : undefined
  );
