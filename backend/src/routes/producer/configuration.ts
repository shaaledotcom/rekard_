...
router.put('/payment-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
router.delete('/payment-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
// ===== Domain Settings =====
router.get('/domain', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
  const settings = await configService.getDomain(tenant.appId, tenant.tenantId, tenant.userId);
  ok(res, settings);
}));

router.put('/domain', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
router.delete('/domain', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
// ===== SMS Gateway =====
router.get('/sms-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
  const settings = await configService.getSmsGateway(tenant.appId, tenant.tenantId, tenant.userId);
  ok(res, settings);
}));

router.put('/sms-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
router.delete('/sms-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
// ===== Email Gateway =====
router.get('/email-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
  const settings = await configService.getEmailGateway(tenant.appId, tenant.tenantId, tenant.userId);
  ok(res, settings);
}));

router.put('/email-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
router.delete('/email-gateway', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
// ===== Payment Receiver =====
router.get('/payment-receiver', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
  const settings = await configService.getPaymentReceiver(tenant.appId, tenant.tenantId, tenant.userId);
  ok(res, settings);
}));

router.put('/payment-receiver', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
...
router.delete('/payment-receiver', asyncHandler(async (req: AppRequest, res: Response) => {
  const tenant = getTenantContext(req);
