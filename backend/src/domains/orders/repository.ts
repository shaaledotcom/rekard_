...
// Get user purchases with ticket details across all tenants (for cross-domain purchase checking)
// Orders can be created from any domain (custom domain or watch.rekard.com)
export const listUserPurchasesWithTicketDetailsAcrossTenants = async (
  userId: string,
  pagination: PaginationParams = {}
) => {
  const page = Math.max(1, pagination.page || 1);
