...
function CreateEventCard({ onClick }: { onClick: () => void }) {
  return (
    <motion.button
      onClick={onClick}
      className="group relative h-full min-h-[280px] rounded-2xl border-2 border-dashed border-border hover:border-red-500/50 bg-gradient-to-br from-muted/30 to-muted/50 hover:from-red-500/5 hover:to-red-500/10 transition-all duration-300 flex flex-col items-center justify-center gap-4 cursor-pointer overflow-hidden"
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="relative w-16 h-16 rounded-2xl bg-muted/50 group-hover:bg-red-500 border border-border group-hover:border-red-400 flex items-center justify-center transition-all duration-300 shadow-lg group-hover:shadow-red-500/25">
      ...
      <Sparkles className="absolute top-4 right-4 h-5 w-5 text-transparent group-hover:text-amber-400 transition-all duration-300" />
    </motion.button>
  );
}
